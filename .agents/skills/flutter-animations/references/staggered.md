...
class MultiPropertyStagger extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: controller,
      builder: (context, child) {
        final animatedColor = (color.value ?? Colors.transparent).withValues(
          alpha: opacity.value,
        );
        return Container(
          width: width.value,
...
// Use alpha on paintable elements when possible
Container(color: Colors.blue.withValues(alpha: animations[i].value))
```
